...
import { DropContainer, Mes, MessagesAreaWrapper, ScrollableDiv, ScrollBottom, SimpleBarWrapper } from "./MessagesArea.style"
...
const MessagesArea = ({currUser, messages, chatId, getMessages, openModal, onLoad}) => {
	...
	// 	}
		...
		return () => {
			...
			refElem?.current?.removeEventListener("scroll", scrollBottom)
		}
	...
	return (
			<DropContainer {...getRootProps({ isDragAccept, isFocused})}>
        		...
        		<input {...getInputProps()} />
				<MessagesAreaWrapper>
					<ScrollableDiv id="scrollableDiv" ref={refElem}>
						<InfiniteScroll
					...
					<ScrollBottom isVisible={isVisible}><img style={{width: '50px', height: '50px'}} onClick={() => refElem.current.scrollTop = 0} src={ScrollBtn}/></ScrollBottom>
					{/* <SimpleBarWrapper >  */}
...
export default connect(state=>({currUser: state.auth?.payload?.sub?.id}), {getMessages : actionGetMessageForChat, openModal: actionOpenModal, onLoad: actionSetDropMedia})(memo(MessagesArea))